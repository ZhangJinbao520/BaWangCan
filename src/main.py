...
from area import dictCity, dictProvince
from backendThread import linkStatusThread, runResultThread, runStatusThread
...
class Ui_Login(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        self.setStyleSheet(
            "background-color: rgba(255, 255, 255, 255);border-width:0;border-style:outset;"
        )
        # 应用图标
        ico = "https://www.dpfile.com/app/pc-common/dp_favicon.a4af753914321c8e82e402e2b4be01d7.ico"
        icon = QtGui.QIcon()
        icon.addPixmap(
            QtGui.QPixmap(self.getImgFromInternet(ico)["img"]),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
        ...
        self.contentQRCodeInvalid.setStyleSheet(
        ...
        self.btnRefresh.setCursor(QtGui.QCursor(
            QtCore.Qt.PointingHandCursor))  # 设置鼠标手势
        self.btnRefresh.setStyleSheet(
            "font: 12pt bold;color: rgb(255, 255, 255);background-color: rgba(255, 102, 51, 240);"
        )
        self.btnRefresh.setObjectName("btnRefresh")
        # 信号关联槽位
        QtCore.QMetaObject.connectSlotsByName(
            self)  # 自动连接槽，使得装饰器@QtCore.pyqtSlot可生效
    ...
    def setQRCodeFromImg(self):
        self.QRCode = self.getImgFromInternet(
            "https://account.dianping.com/account/getqrcodeimg"
        )
    ...
    def queryQRCodeStatus(self, status):
        ...
        if status == 0:
            ...
            self.close()
            self.UI = TrayIcon(
                Config().getConfig("Cookie", "Cookie")
            )  # 必须是self.UI，不能是UI？？？【暂时不知道为什么】
        else:
            # 连接超时、或用户取消登录、或异常
            if status == 3:
                # 用户取消登录
    ...
    def retranslateUi(self):
        ...
        self.contentQRCodeInvalid.setHtml(
            _translate(
                "Login",
                "<br><br><br>"
    ...
    def getImgFromBase64(src):
        ...
        return QtGui.QImage.fromData(
            base64.urlsafe_b64decode(src.split(",")[1])
        )  # 返回bytes类型
    ...
    def getImgFromInternet(url):
        ...
        try:
            ...
            lgtoken = re.search(
                r"lgtoken=(.*); ?Domain", response.headers["Set-Cookie"]
            )[1]
        except:
            lgtoken = None
...
class Ui_Run(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(
            QtGui.QPixmap(Ui_Login.getImgFromInternet(ico)["img"]),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
        self.setWindowIcon(icon)
        ...
        self.contentUser.setVerticalScrollBarPolicy(
        ...
        self.iconLevel.setPixmap(
            QtGui.QPixmap(Ui_Login.getImgFromBase64(self.userLevel))
        )
        ...
        self.boxProvince.setStyleSheet(
            'font: 11pt "华文楷体";border: 1px solid rgb(171, 171, 171);border-radius: 2px;'
        )
        ...
        self.btnRun.setCursor(QtGui.QCursor(
            QtCore.Qt.PointingHandCursor))  # 设置鼠标手势
        # @ 2021/02/04，设置按钮移动变色
        self.btnRun.setStyleSheet(
            "QPushButton:hover { font: 16pt bold;text-decoration: underline;;background-color: rgba(242, 242, 242, 255);border: 1px solid rgb(150, 150, 150);border-radius: 6px;} QPushButton { font: 16pt bold;background-color: rgba(254, 254, 254, 255);border: 1px solid rgb(150, 150, 150);border-radius: 6px;}"
        )
        # @ 2021/02/04，设置按钮移动变色
        # 信号关联槽位
        QtCore.QMetaObject.connectSlotsByName(
        ...
        self.backendResult = runResultThread(
            self.Cookie, self.userNickName)  # 创建线程_2
    ...
    def setImgCycle(self, url):
        ...
        img = QtGui.QPixmap(Ui_Login.getImgFromInternet(url)["img"]).scaled(
            100,
            100,
    ...
    def on_btnRun_clicked(self):
        ...
        if self.boxCity.currentText() != "请选择":
            ...
            self.backendResult.CityId = dictCity[self.boxProvince.currentText()][
                self.boxCity.currentText()
            ]
    ...
    def retranslateUi(self):
        ...
        self.contentUseday.setHtml(
            _translate(
                ...
                "<span style=\" font-family:'PingFangSC-Regular','Microsoft YaHei','Hiragino Sans GB','Helvetica'; font-size:12pt; color:#ff6633; background-color:#ffffff; vertical-align:middle;\">"
                + str(self.useDpDays)
                + "</span>"
        ...
        self.contentOpt.setHtml(
            _translate(
                ...
                '<span style=" font-size:13pt bold;"><a href=http://www.dianping.com/member/'
                + str(self.userId)
                + "/wishlists>"
                + str(self.favorShopCount)
                + "</a></span>"
                ...
                '<span style=" font-size:13pt bold;"><a href=http://t.dianping.com/account/coupons>'
                + str(self.dealCount)
                + "</a></span></p>",
            )
        )
        self.contentUser.setHtml(
            _translate(
                "Run",
                "<p align=\"right\"><span style=\" font-family:'PingFangSC-Regular','Microsoft YaHei','Hiragino Sans GB','Helvetica'; font-size:11pt; color:#333333; background-color:#ffffff; vertical-align:middle;\">"
                + hello
                + "</span>"
                '<span style=" font-size:13pt bold;"><a href=http://www.dianping.com/member/'
                + str(self.userId)
...
class TrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def createTaskbarMenu(self):
        ...
        self.showMenu = QtWidgets.QAction(
            "显示/隐藏", self, triggered=self.on_showMenu_clicked
        )  # 定义菜单名称以及关联事件
        self.quitMenu = QtWidgets.QAction(
            "退出     ", self, triggered=self.on_quitMenu_clicked
        )
