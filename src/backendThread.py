...
class runResultThread(QThread):
    ...
    def run(self):
        ...
        for _activity in activityTitles:
            ...
            # @ 2021/02/23，修订推文样式
            self.MESSAGE += "{0}、{1}\n".format(count,
                                               _activity["activityTitle"])
            # @ 2021/02/23，修订推文样式
            offlineActivityId = _activity["detailUrl"].replace(
                "http://s.dianping.com/event/", ""
            )
            self.Database[count]["applyResult"] = self.runBaWangCan(
                offlineActivityId)
            self.MESSAGE += " - 【报名结果】：{}\n\n".format(
                self.Database[count]["applyResult"]
            )
        ...
        self.MESSAGE = (
            "---\n\n-----开始报名霸王餐（免费试）-----\n\n用户名：***{0}***\n\n城 市：***{1}***\n\n- 今日报名成功：**{2}**\n\n- 今日报名重复：**{3}**\n\n- 今日报名异常：**{4}**\n\n---\n\n-----今日成果预览-----\n\n".format(
                self.userNickName, self.City, self.PASS, self.SKIP, self.FAIL
            )
            + self.MESSAGE
        )
        # @ 2021/02/23，修订推文样式
    ...
    def getBaWangCanDetail(self, url):
        ...
        if response.status_code == 200:
            detail["activityAddress"] = re.search(
                r"活动地址：</span>\s+(.*)\s", response.text
            )[1]
            try:
                detail["applyStartTime"] = re.search(
                    r"报名时间：</span>(\d+月\d+日).*(\d+月\d+日).*", response.text
                )[1]
                detail["applyEndTime"] = re.search(
                    r"报名时间：</span>(\d+月\d+日).*(\d+月\d+日).*", response.text
                )[2]
            except:
                detail["applyStartTime"] = re.search(
                    r"报名时间：</span>(\d+月\d+日).*", response.text
                )[1]
                detail["applyEndTime"] = ""
            try:
                detail["activityStartTime"] = re.search(
                    r"活动时间：</span>(\d+月\d+日).*(\d+月\d+日).*</li>", response.text
                )[1]
                detail["activityEndTime"] = re.search(
                    r"活动时间：</span>(\d+月\d+日).*(\d+月\d+日).*</li>", response.text
                )[2]
            except:
                detail["activityStartTime"] = re.search(
                    r"活动时间：</span>(\d+月\d+日).*</li>", response.text
                )[1]
                detail["activityEndTime"] = ""
            detail["activityCount"] = re.search(
                r'活动名额：</span>\s+<strong class="col-digit">(\d+)</strong> 个',
                response.text,
            )[1]
            try:
                detail["passCount"] = re.search(
                    r"支持pass卡（剩余(\d+)个）", response.text)[1]
            except:
                detail["passCount"] = "不支持"
            detail["applyCount"] = re.search(
                r"<strong>(\d+)</strong>人报名", response.text
            )[1]
            detail["attentionCount"] = re.search(
                r"<strong>(\d+)</strong>人关注", response.text
            )[1]
            detail["winningRate"] = "{0:.2f}".format(
                int(detail["activityCount"]) / int(detail["applyCount"]) * 100
            )
        return detail
    ...
    def weixinTrap(self):
        ...
        SCKEY = (
            "SCU155771T3549c0427011a83c02d53a4f054055166012211d21350"  # Server酱申请的SCKEY
        )
